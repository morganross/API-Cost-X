...
export default function ExactPresetListView({
  matrixModels,
  distinctProviders,
  ...
  const SECTION_META: Record<MatrixSection | 'aiq', { label: string; glow: string; onClass: string; offClass: string }> = {
    fpf: {
      label: 'FPF',
  ...
  const sortModels = (models: string[]) => {
    if (!cardSortCol) return models
    if (cardSortCol === 'score') return sortByOptionalMetric(models, selectedGenerationScore, (model) => model, cardSortDir)
    if (cardSortCol === 'evalScore') return sortByOptionalMetric(models, evalScore, (model) => model, cardSortDir)
    return sortByOptionalMetric(models, (model) => model, (model) => model, cardSortDir)
  }

  const isModelVisible = (model: string) => {
  ...
  const allModels = sortModels(matrixModels)
  const visibleModels = allModels.filter(isModelVisible)
  ...
  return (
    <div className="bg-gray-800 border border-gray-700 rounded-lg overflow-hidden mb-6" data-testid="preset14-view-list">
      ...
      <div className="p-4">
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 pb-3 mb-3 border-b border-gray-700 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={() => setShowScored((value) => !value)}
              className={`px-2 py-0.5 rounded-full ${showScored ? `${pillOn} bg-gray-600 text-gray-100 border-gray-500` : pillOff}`}
            >
              Scored
            </button>
            <button
              onClick={() => setShowUnscored((value) => !value)}
              className={`px-2 py-0.5 rounded-full ${showUnscored ? `${pillOn} bg-gray-600 text-gray-100 border-gray-500` : pillOff}`}
            >
              Unscored
            </button>
            <button
              onClick={() => setShowFree((value) => !value)}
              className={`px-2 py-0.5 rounded-full ${showFree ? `${pillOn} bg-green-700 text-green-100 border-green-600` : pillOff}`}
            >
              Free
            </button>
          </div>
          ...
          <div className="flex items-center gap-1.5">
            <button
              onClick={() => setHideKeyless((value) => !value)}
              className={`px-2 py-0.5 rounded-full ${hideKeyless ? `${pillOn} bg-yellow-700 text-yellow-100 border-yellow-600` : pillOff}`}
            >
              BYOK only
            </button>
          </div>
        ...
        {visibleModels.length === 0 ? (
          ...
          <div className="grid justify-center gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, 58rem)' }}>
            {visibleModels.map((model) => {
              const isFree = hasFreeSection(models[model]?.sections) || model.includes(':free')
              const selectedScore = selectedGenerationScore(model)

              const renderPhaseButton = (section: MatrixSection | 'aiq') => {
                  ...
                  >
                    <div className="flex items-center justify-center gap-2">
                      <div className="relative h-7 w-7 shrink-0 overflow-visible">
                        ...
                        {checked ? (
                          <>
                            <Check className="pointer-events-none absolute -left-3 -top-3 z-20 h-14 w-14 text-black" strokeWidth={5} />
                            <Check className="pointer-events-none absolute -left-3 -top-3 z-[21] h-14 w-14 text-white" strokeWidth={3.5} />
                          </>
                        ) : null}
                      </div>
                      <div className={cn('font-semibold uppercase tracking-[0.22em]', section === 'combine' ? 'text-xs' : 'text-[10px]')}>
                        {meta.label}
                      </div>
                    </div>
                  </button>
              ...
              return (
                <div
                  key={model}
                  className="preset4-card w-[58rem] rounded-[24px] border border-gray-700 bg-[linear-gradient(180deg,rgba(31,41,55,0.98)_0%,rgba(17,24,39,0.98)_100%)] transition-transform duration-200"
                >
                  <div className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="flex shrink-0 flex-nowrap items-center gap-2">
                        {renderPhaseButton('fpf')}
                        <div
                          className={cn(
                            'inline-flex h-12 w-12 items-center justify-center rounded-md px-0 text-center text-[10px] leading-tight',
                            selectedScore != null ? tierClass(selectedScore, q.genScore, true) : 'bg-slate-800 text-gray-400'
                          )}
                        >
                          {selectedScore != null ? `${((selectedScore / 5) * 100).toFixed(0)}%` : '--'}
                        </div>
                        {renderPhaseButton('gptr')}
                        {renderPhaseButton('dr')}
                        {renderPhaseButton('aiq')}
                      </div>
                      <div className="flex h-12 w-44 shrink-0 items-center text-sm font-mono text-white">
                        <h4
                          className="w-full break-words text-[13px] leading-5"
                          style={{
                            display: '-webkit-box',
                            WebkitLineClamp: 2,
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        <div className="flex items-center gap-2 text-[11px] font-mono">
                          <div
                            className={cn(
                              'inline-flex h-12 w-12 items-center justify-center rounded-md px-0 text-center text-[10px] leading-tight',
                              evalScore(model) != null ? tierClass(evalScore(model) as number, q.evalScore, true) : 'bg-slate-800 text-gray-400'
                            )}
                          >
                            {evalScore(model) != null ? `${(evalScore(model) as number).toFixed(0)}%` : '--'}
                          </div>
                        </div>
                        {renderPhaseButton('combine')}
                      </div>
                      <div className="ml-auto flex shrink-0 items-center justify-end gap-2 text-[11px] font-mono">
                        {isFree ? (
